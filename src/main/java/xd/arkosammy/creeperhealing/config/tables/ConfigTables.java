...
public enum ConfigTables {
    DELAYS_CONFIG_TABLE(new SimpleConfigTable("delays", """
            Configure the delays related to the healing of explosions.""", DelaysSettings.getConfigSettings())),
    EXPLOSION_ITEM_DROPS_TABLE(new SimpleConfigTable("explosion_item_drops", """
            Toggle whether certain explosion should drop items. Does not include items stored in container blocks.""", ExplosionItemDropSettings.getConfigSettings())),
    EXPLOSION_SOURCE_TABLE(new SimpleConfigTable("explosion_sources", """
            Configure which explosions are allowed to heal.""", ExplosionSourceSettings.getConfigSettings())),
    HEALING_MODE_TABLE(new SimpleConfigTable("explosion_healing_mode", """
            Choose between different special modes for explosion healing. Note that certain healing modes will not follow the explosion delay and block delay settings.""", HealingModeSettings.getConfigSettings())),
    PREFERENCES_TABLE(new SimpleConfigTable("preferences", """
            Toggleable settings for extra features.""", PreferencesSettings.getConfigSettings())),
    WHITELIST_TABLE(new SimpleConfigTable("whitelist", """
            ...
            Example entries:
            whitelist_entries = ["minecraft:grass",  "minecraft:stone", "minecraft:sand"]""", WhitelistSettings.getConfigSettings())),
