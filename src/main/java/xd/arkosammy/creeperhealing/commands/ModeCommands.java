...
public final class ModeCommands {
    ...
    private static int setDefaultHealingModeCommand(CommandContext<ServerCommandSource> ctx){
        ConfigManager.getInstance().getAsStringSetting(ConfigSettings.MODE.getId()).setValue(ExplosionHealingMode.DEFAULT_MODE.getName());
    ...
    private static int setDaytimeHealingModeCommand(CommandContext<ServerCommandSource> ctx){
        ConfigManager.getInstance().getAsStringSetting(ConfigSettings.MODE.getId()).setValue(ExplosionHealingMode.DAYTIME_HEALING_MODE.getName());
    ...
    private static int setDifficultyBasedModeCommand(CommandContext<ServerCommandSource> ctx){
        ConfigManager.getInstance().getAsStringSetting(ConfigSettings.MODE.getId()).setValue(ExplosionHealingMode.DIFFICULTY_BASED_HEALING_MODE.getName());
    ...
    private static int setBlastResistanceBasedHealingModeCommand(CommandContext<ServerCommandSource> ctx){
        ConfigManager.getInstance().getAsStringSetting(ConfigSettings.MODE.getId()).setValue(ExplosionHealingMode.BLAST_RESISTANCE_BASED_HEALING_MODE.getName());
    ...
    private static int getHealingModeCommand(CommandContext<ServerCommandSource> ctx){
        ctx.getSource().sendMessage(Text.literal("Explosion healing mode currently has been set to: " + ExplosionHealingMode.getFromName(ConfigManager.getInstance().getAsStringSetting(ConfigSettings.MODE.getId()).getValue()).getDisplayName()));
        return Command.SINGLE_SUCCESS;
    }
