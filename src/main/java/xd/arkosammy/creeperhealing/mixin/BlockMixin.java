...
@Mixin(Block.class)
public abstract class BlockMixin extends AbstractBlock implements BlockAccessor {
    ...
    private boolean shouldExplosionDropItems(boolean original, @Local Explosion explosion){
        Entity causingEntity = ((ExplosionAccessor)explosion).creeper_healing$getEntity();
        ...
        if (causingLivingEntity instanceof CreeperEntity && ExplosionItemDropConfig.DROP_ITEMS_ON_CREEPER_EXPLOSIONS.getEntry().getValue()){
            ...
            shouldDropItems = true;
        } else if (damageSource instanceof BadRespawnPointDamageSource && ExplosionItemDropConfig.DROP_ITEMS_ON_BED_AND_RESPAWN_ANCHOR_EXPLOSIONS.getEntry().getValue()){
            shouldDropItems = true;
