...
public record SerializedExplosionEvent(String healingMode, List<SerializedAffectedBlock> serializedAffectedBlocks, long healTimer, int blockCounter) {
    ...
    public static final Codec<SerializedExplosionEvent> CODEC = RecordCodecBuilder.create(instance -> instance.group(
     ...
     public ExplosionEvent toDeserialized(){
        final List<AffectedBlock> affectedBlocks = this.serializedAffectedBlocks.stream().map(SerializedAffectedBlock::toDeserialized).collect(Collectors.toList());
        final ExplosionHealingMode explosionHealingMode = ExplosionHealingMode.getFromName(this.healingMode);
